# ruff: noqa
# =============================================================================
# VENDORED MODULE — DO NOT EDIT THE ALGORITHMS.
#
# Source: research/binderjet/code/rfam_tool/feature_analysis.py
# Copied: 2026-09-14 (Lane A — Dimensional Accuracy & Compensation, v1).
# Provenance: verbatim copy of the RFAM metrology tool's analyze_*/recommend_*
#   functions and their private helpers. Kept self-contained so the console does
#   not depend on the unpackaged rfam_tool repo.
#
# Deviations from the source (each required to make the copy runnable + correct
# on this environment; none change the algorithms or the numbers):
#   1. `_nan_ring_result(...)` is CALLED by analyze_concentric_rings in the source
#      but its definition is MISSING there (latent NameError on any ring error
#      path). It is restored verbatim from the sibling backup that still defines
#      it: code/rfam_tool/Upgrades/rfam_tool_final_final/backup-OLD/feature_analysis.py:282.
#   2. PCA eigenvectors from `np.linalg.eig(cov)` come back with complex dtype on
#      numpy>=2, and `np.arctan2` rejects complex input (crashes the checkerboard;
#      warns in the dot path). We take `.real` at the two extraction sites. The
#      source already discards the (zero) imaginary part via `float()`, so this is
#      the same truncation made explicit; it reproduces the fixture numbers exactly.
# =============================================================================
"""
...
def _compute_spacing_pairwise(
    ...
    if nominal_spacing_mm is not None and not (isinstance(nominal_spacing_mm, float) and math.isnan(nominal_spacing_mm)):
        ...
        v1 = vecs[:, int(np.argmax(vals))].real  # VENDOR FIX #2: coerce complex eig output to real
...
def analyze_checkerboard(
    ...
    for sc in scales:
        ...
        vals, vecs = np.linalg.eig(cov)
        v = vecs[:, int(np.argmax(vals))].real  # VENDOR FIX #2: coerce complex eig output to real
