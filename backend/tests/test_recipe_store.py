...
from vention_printer_interface.control.recipe_store import CONFIG_NAME, load_recipe, save_recipe
from vention_printer_interface.control.safety import SafetyLimits


def test_round_trip(tmp_path: Path) -> None:
    plan = RecipePlan.bounded({"printing": {"n_layers": 3}, "heater_enabled": True}, SafetyLimits())
    save_recipe(tmp_path, plan)
    assert (tmp_path / CONFIG_NAME).exists()
    assert load_recipe(tmp_path, SafetyLimits()) == plan


def test_missing_or_corrupt_gives_defaults(tmp_path: Path) -> None:
    assert load_recipe(tmp_path, SafetyLimits()) == RecipePlan()
...
def test_hand_edited_is_rebounded(tmp_path: Path) -> None:
    ...
    p = load_recipe(tmp_path, SafetyLimits())
