...
def wait_recipe(c: TestClient, state: str, timeout: float = 30.0) -> dict[str, Any]:
    ...
    while time.monotonic() < end:
        ...
        if s["state"] == state:
            return s
        time.sleep(0.05)
    raise AssertionError(f"recipe never reached {state}: {c.get('/api/status').json()['recipe']}")


def test_get_recipe_has_plan_bounds_validation_and_steps(client: TestClient) -> None:
...
def test_put_recipe_rebounds_validates_and_persists(client: TestClient, tmp_path: Path) -> None:
    r = client.put("/api/recipe", json={"printing": {"n_layers": 3, "recoater_speed": 99999}})
    ...
    bad = client.put("/api/recipe", json={"printing": {"n_layers": 200, "layer_thickness_mm": 2}})
...
def test_start_refused_when_not_armed_or_invalid(client: TestClient) -> None:
    assert client.post("/api/recipe/start", json={}).status_code == 409
    client.post("/api/connect", json={"backend": "simulated"})
    assert client.post("/api/recipe/start", json={}).status_code == 409
    client.put("/api/recipe", json={"printing": {"n_layers": 200, "layer_thickness_mm": 2}})
    connect_arm(client)
    r = client.post("/api/recipe/start", json={})
...
def test_run_to_done_with_auto_log_and_layers(client: TestClient) -> None:
    client.put("/api/recipe", json=FAST)
    connect_arm(client)
    r = client.post("/api/recipe/start", json={"dry_run": False})
    assert r.status_code == 200 and r.json()["state"] == "running"
    assert client.get("/api/status").json()["recording"]["active"] is True  # auto-log opened
    done = wait_recipe(client, "done")
    ...
    assert "recipe_started" in labels and "layer_completed" in labels and "recipe_done" in labels


def test_pause_resume_abort(client: TestClient) -> None:
    client.put("/api/recipe", json={**FAST, "printing": {**FAST["printing"], "n_layers": 3}})
    connect_arm(client)
    assert client.post("/api/recipe/start", json={}).status_code == 200
    assert client.post("/api/recipe/pause").status_code == 200
    wait_recipe(client, "paused")
    assert client.post("/api/recipe/resume").status_code == 200
    wait_recipe(client, "running")
    r = client.post("/api/recipe/abort")
...
def test_dry_run_and_single_step(client: TestClient) -> None:
    client.put("/api/recipe", json=FAST)
    connect_arm(client)
    r = client.post("/api/recipe/start", json={"dry_run": True, "single_step": True})
    assert r.status_code == 200 and r.json()["dry_run"] is True
    wait_recipe(client, "paused")
    assert client.post("/api/recipe/step").status_code == 200
    wait_recipe(client, "paused")
    assert client.post("/api/recipe/resume").status_code == 200
...
def test_auto_log_toggle(client: TestClient) -> None:
    ...
    client.put("/api/recipe", json=FAST)
    connect_arm(client)
    client.post("/api/recipe/start", json={"dry_run": True})
    assert client.get("/api/status").json()["recording"]["active"] is False


def test_estop_aborts_recipe(client: TestClient) -> None:
    client.put("/api/recipe", json={**FAST, "printing": {**FAST["printing"], "n_layers": 5}})
    connect_arm(client)
    client.post("/api/recipe/start", json={})
    client.post("/api/estop")
    s = wait_recipe(client, "aborted", timeout=5)
