import { TRAVEL_MM, type AxisNo, type StatusPayload } from "../lib/telemetry.ts";
...
export function DockMachine({ status }: { status: StatusPayload | null }) {
  ...
  // gantry rails span x 46..270; carriage centre = rail start + frac*span
  const railX0 = 52, railX1 = 268, span = railX1 - railX0;
  const phX = railX0 + frac(3) * span;
  const rcX = railX0 + frac(4) * span;
  ...
  return (
    <svg className="dockdiag" viewBox="0 0 300 244" role="img" aria-label="machine positions">
      ...
      <text className="lbl dim" x="16" y="96">RC</text>
