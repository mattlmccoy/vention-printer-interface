        ...
        <div className="body">
          {plan ? (
            <div className="fields adv" style={{ marginTop: 0 }}>
              ...
              <span data-tip="Build-piston anti-backlash: overshoot each layer drop DOWN by this much then return UP to the target, so the drop is approached one-sided and lost-motion is taken up. The piston sweep measured ~0.15 mm and it cut mean step error 6.5×. 0 = off (single V1.py move).">build backlash</span><span className="row"><input type="number" step="0.05" value={plan.build_backlash_mm} disabled={running} onChange={(e) => edit({ build_backlash_mm: num(e.target.value, plan.build_backlash_mm) })} /> mm</span>
