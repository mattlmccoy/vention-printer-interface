          ...
          {plan ? (
            <>
              ...
              <div className="fields" style={{ marginTop: 16, maxWidth: "none" }}>
                ...
                <span title="Emit layerwise vision capture marks (pre-jet / post-jet / post-heat) for the camera on the recoater gantry. Adds NO motion — just marks the recording points. Leave OFF until cameras are installed.">layer captures</span><label className="row"><Toggle checked={plan.capture_stages} disabled={running} onChange={(v) => edit({ capture_stages: v })} /> <span className="hint">record the recoater-gantry camera at each layer stage (no cameras installed yet — leave off)</span></label>
              </div>
        ...
        <div className="body">
          {plan ? (
            <div className="fields adv" style={{ marginTop: 0 }}>
              ...
              <span title="Reference position where the printhead dwells for a nozzle purge (unless a purge position is set). The printhead is no longer parked here at setup — it stays home until it jets, mm.">printhead start</span><span className="row"><input type="number" value={plan.printhead_start_mm} disabled={running} onChange={(e) => edit({ printhead_start_mm: num(e.target.value, plan.printhead_start_mm) })} /> mm</span>
