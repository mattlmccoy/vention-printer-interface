    ...
    { id: "stack", title: "powder stack", size: "m", node: plan ? (
      <>
        ...
        <div className="fields" style={{ marginTop: 16, maxWidth: "none" }}>
          ...
          <span>heater</span><label className="row"><input type="checkbox" checked={plan.heater_enabled} disabled={running} onChange={(e) => edit({ heater_enabled: e.target.checked })} /> dry with <input type="number" value={plan.n_heater_passes} disabled={running || !plan.heater_enabled} onChange={(e) => edit({ n_heater_passes: num(e.target.value, plan.n_heater_passes) })} style={{ width: 64 }} /> pass(es)</label>
        </div>
