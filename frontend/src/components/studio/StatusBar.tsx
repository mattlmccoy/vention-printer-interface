...
export interface StatusBarProps {
  ...
  recActive: boolean; recRun: string | null; recipeState: string; version: string | null; apiWarning: string | null;
}

/** Bottom status bar (FLIR rule): never shows green; faults red, unknowns amber. Presentational. */
export function StatusBar(p: StatusBarProps) {
  ...
  return (
    <footer className="statusbar">
      ...
      {p.apiWarning && <span className="bad">{p.apiWarning}</span>}
      <span className="right">
        ...
        {p.version && <span className="muted" title="Vention Printer Interface operator version">v{p.version}</span>}
      </span>
    </footer>
  );
}
