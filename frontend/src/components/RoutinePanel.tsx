...
export function RoutinePanel({ gates, call }: { gates: Gates; call: Call }) {
  ...
  const pos = (label: string, key: keyof Draft, title?: string) => (
    <div className="rp-row"><span data-tip={title}>{label}</span><span className="rv"><NumberField value={d![key] as number} disabled={!ok} onChange={(v) => setD({ [key]: v } as Partial<Draft>)} /></span></div>
  );
    ...
    <>
      <div className="rp-grid">
        <div className="rp-card">
          ...
          <div className="rp-row"><span data-tip="Thin precoat layers laid before printing — count × layer thickness (mm). Kept thin to establish a fresh, level bed.">thin precoat (n × mm)</span><span className="rv"><NumberField value={d.thin_precoat.n_layers} disabled={!ok} style={{ width: 46 }} onChange={(v) => setPh("thin_precoat", { n_layers: v })} /> × <NumberField step="0.1" value={d.thin_precoat.layer_thickness_mm} disabled={!ok} style={{ width: 64 }} onChange={(v) => setPh("thin_precoat", { layer_thickness_mm: v })} /></span></div>
          <div className="rp-row"><span data-tip="Powder fed up from the feed piston for each thin-precoat layer (mm) before the recoater spreads it.">precoat feed (mm)</span><span className="rv"><NumberField step="0.1" value={d.thin_precoat.feed_thickness_mm} disabled={!ok} onChange={(v) => setPh("thin_precoat", { feed_thickness_mm: v })} /></span></div>
          <div className="rp-row"><span data-tip="Powder fed up per printing layer (mm) — usually a bit more than the layer height so the recoater always has enough to spread.">printing feed (mm)</span><span className="rv"><NumberField step="0.1" value={d.printing_feed_thickness_mm} disabled={!ok} onChange={(v) => setD({ printing_feed_thickness_mm: v })} /></span></div>
          <div className="rp-row"><span data-tip="Absolute recoater position (mm) it returns to after spreading a precoat layer (not fully home).">recoater return (mm)</span><span className="rv"><NumberField value={d.recoater_return_mm} disabled={!ok} onChange={(v) => setD({ recoater_return_mm: v })} /></span></div>
          <div className="rp-row"><span data-tip="Anti-backlash: drops the feed piston this much BEFORE the recoater spread, then the post-spread feed-up returns it from below to take up mechanical slop. 0 = off.">feed backlash (mm)</span><span className="rv"><NumberField step="0.1" value={d.feed_backlash_mm} disabled={!ok} onChange={(v) => setD({ feed_backlash_mm: v })} /></span></div>
          <div className="rp-row"><span data-tip="Spread capping layers over the finished part after the last printing layer.">postcoat</span><span className="rv"><Toggle checked={d.postcoat_enabled} disabled={!ok} onChange={(v) => setD({ postcoat_enabled: v })} /></span></div>
          {d.postcoat_enabled && <>
            <div className="rp-row"><span data-tip="Number of postcoat layers × the layer height — just like precoats. The part is held; each layer feeds + spreads powder to cap the build.">postcoat (n × mm)</span><span className="rv"><NumberField value={d.postcoat.n_layers} disabled={!ok} style={{ width: 46 }} onChange={(v) => setPh("postcoat", { n_layers: v })} /> × <NumberField step="0.1" value={d.postcoat.layer_thickness_mm} disabled={!ok} style={{ width: 64 }} onChange={(v) => setPh("postcoat", { layer_thickness_mm: v })} /></span></div>
            <div className="rp-row"><span data-tip="Powder fed up per postcoat layer (mm) before each capping spread.">postcoat feed (mm)</span><span className="rv"><NumberField step="0.1" value={d.postcoat.feed_thickness_mm} disabled={!ok} onChange={(v) => setPh("postcoat", { feed_thickness_mm: v })} /></span></div>
          </>}
        ...
        <div className="rp-card">
          <h4>multipass</h4>
          <div className="rp-row"><span data-tip="prints each layer N times without dropping the build piston">passes / layer</span><span className="rv"><NumberField value={d.n_jet_passes} disabled={!ok} onChange={(v) => setD({ n_jet_passes: v })} /></span></div>
          <div className="rp-row"><span data-tip="Between multipass passes the printhead returns only to here instead of home (saves travel); it still returns home on the last pass.">multipass return (mm)</span><span className="rv"><NumberField value={d.printhead_multipass_return_mm} disabled={!ok} onChange={(v) => setD({ printhead_multipass_return_mm: v })} /></span></div>
          <div className="rp-row"><span data-tip="Where the printhead parks at the start of every print (after the setup homing), before layer 1.">printhead start (mm)</span><span className="rv"><NumberField value={d.printhead_start_mm} disabled={!ok} onChange={(v) => setD({ printhead_start_mm: v })} /></span></div>
        </div>
        ...
        <div className="rp-card">
          ...
          <div className="rp-row"><span data-tip="Build-piston speed / acceleration during printing layers (mm/s · mm/s²).">build (mm/s · mm/s²)</span><span className="rv"><NumberField step="0.1" value={d.printing_part_speed} disabled={!ok} style={{ width: 60 }} onChange={(v) => setD({ printing_part_speed: v })} /> / <NumberField step="1" value={d.printing_part_accel} disabled={!ok} style={{ width: 64 }} onChange={(v) => setD({ printing_part_accel: v })} /></span></div>
          <div className="rp-row"><span data-tip="Feed-piston speed / acceleration during printing layers (mm/s · mm/s²).">feed (mm/s · mm/s²)</span><span className="rv"><NumberField step="0.1" value={d.printing_feed_speed} disabled={!ok} style={{ width: 60 }} onChange={(v) => setD({ printing_feed_speed: v })} /> / <NumberField step="1" value={d.printing_feed_accel} disabled={!ok} style={{ width: 64 }} onChange={(v) => setD({ printing_feed_accel: v })} /></span></div>
          <div className="rp-row"><span data-tip="Printhead-gantry speed / acceleration during printing layers (mm/s · mm/s²).">printhead (mm/s · mm/s²)</span><span className="rv"><NumberField step="1" value={d.printing_printhead_speed} disabled={!ok} style={{ width: 60 }} onChange={(v) => setD({ printing_printhead_speed: v })} /> / <NumberField step="10" value={d.printing_printhead_accel} disabled={!ok} style={{ width: 64 }} onChange={(v) => setD({ printing_printhead_accel: v })} /></span></div>
          <div className="rp-row"><span data-tip="Recoater-gantry speed / acceleration during printing layers (mm/s · mm/s²).">recoater (mm/s · mm/s²)</span><span className="rv"><NumberField step="1" value={d.printing_recoater_speed} disabled={!ok} style={{ width: 60 }} onChange={(v) => setD({ printing_recoater_speed: v })} /> / <NumberField step="10" value={d.printing_recoater_accel} disabled={!ok} style={{ width: 64 }} onChange={(v) => setD({ printing_recoater_accel: v })} /></span></div>
        </div>

        <div className="rp-card">
          <h4>heat</h4>
          <div className="rp-row"><span data-tip="Absolute recoater position (mm) where the IR heater sweep begins.">heater start (mm)</span><span className="rv"><NumberField value={d.heater_start_mm} disabled={!ok} onChange={(v) => setD({ heater_start_mm: v })} /></span></div>
          <div className="rp-row"><span data-tip="Manual recoater sweep speed during the heater pass — overrides the computed exposure sweep (unachievable on current hardware).">heater speed (mm/s)</span><span className="rv"><NumberField step="1" value={d.heater_speed} disabled={!ok} onChange={(v) => setD({ heater_speed: v })} /></span></div>
          <div className="rp-row"><span data-tip="Recoater acceleration during the heater sweep (paired with heater speed).">heater accel (mm/s²)</span><span className="rv"><NumberField step="10" value={d.heater_accel} disabled={!ok} onChange={(v) => setD({ heater_accel: v })} /></span></div>
          <div className="rp-row"><span data-tip="Extra build-piston drop (mm) before the heater sweep, raised back after — net descent stays one layer. 0 = off.">pre-heater drop (mm)</span><span className="rv"><NumberField step="0.1" value={d.pre_heater_drop_mm} disabled={!ok} onChange={(v) => setD({ pre_heater_drop_mm: v })} /></span></div>
        </div>

        <div className="rp-card">
          <h4>nozzle purge</h4>
          <div className="rp-row"><span data-tip="Firing is external, so this holds the printhead for this many seconds before a jet pass — a window for the printhead controller to purge. 0 = off.">purge dwell (s)</span><span className="rv"><NumberField step="0.1" value={d.purge_dwell_s} disabled={!ok} style={{ width: 64 }} onChange={(v) => setD({ purge_dwell_s: v })} /></span></div>
          <div className="rp-row"><span data-tip="Absolute printhead position (mm) where the purge dwell holds. Blank = use the printhead start position.">purge position (mm)</span><span className="rv"><input type="text" style={{ width: 80, textAlign: "right" }} placeholder={`${d.printhead_start_mm} (start)`} value={d.purge_position_mm ?? ""} disabled={!ok} onChange={(e) => setD({ purge_position_mm: e.target.value === "" ? null : Number(e.target.value) })} /></span></div>
          <div className="rp-row" style={{ alignItems: "flex-start" }}><span data-tip="How often the nozzle-purge dwell runs: every jet pass, once per layer, or every N layers.">frequency</span><span className="seg">{([["every_pass", "every pass"], ["per_layer", "per layer"], ["every_n_layers", "every N"]] as const).map(([m, lbl]) => <button key={m} type="button" className={`small${d.purge_mode === m ? " on" : ""}`} aria-pressed={d.purge_mode === m} disabled={!ok} onClick={() => setD({ purge_mode: m })}>{lbl}</button>)}</span></div>
          {d.purge_mode === "every_n_layers" && <div className="rp-row"><span data-tip="Run the purge dwell every N printing layers (used with the 'every N' frequency).">N (layers)</span><span className="rv"><NumberField value={d.purge_every_n_layers} disabled={!ok} style={{ width: 56 }} onChange={(v) => setD({ purge_every_n_layers: v })} /></span></div>}
        </div>

        <div className="rp-card">
          <h4>carbon &amp; exposure</h4>
          <div className="rp-row"><span data-tip="Target binder carbon content (weight fraction) — drives the computed IPA heater exposure.">target carbon (wt)</span><span className="rv"><NumberField step="0.01" value={d.target_carbon_wt} disabled={!ok} onChange={(v) => setD({ target_carbon_wt: v })} /></span></div>
          <div className="rp-row"><span data-tip="Part cross-sectional area (mm²) used in the heater-exposure model.">part area (mm²)</span><span className="rv"><NumberField value={d.part_area_mm2} disabled={!ok} onChange={(v) => setD({ part_area_mm2: v })} /></span></div>
          <div className="rp-row"><span data-tip="IR heater section power (W) used in the exposure model.">heater power (W)</span><span className="rv"><NumberField value={d.heater_section_power_w} disabled={!ok} onChange={(v) => setD({ heater_section_power_w: v })} /></span></div>
        ...
        <div className="rp-card">
          <h4>finish</h4>
          <div className="rp-row"><span data-tip="FINISH drives the part cylinder to this absolute position — the spill-safe depth.">part drop-to (mm)</span><span className="rv"><NumberField value={d.part_max_mm} disabled={!ok} onChange={(v) => setD({ part_max_mm: v })} /></span></div>
