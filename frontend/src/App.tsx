...
export function App() {
  ...
  const [opPlatform, setOpPlatform] = useState<string | null>(null); // operator's OS (health.platform)
  ...
  useEffect(() => {
    ...
    const open = () => {
      ...
      ws.onopen = () => { setReachable(true); setWsFails(0); api.health().then((h) => { setVersion(h.version); setOpBuild(h.build ?? null); setOpPlatform(h.platform ?? null); const hs = checkHandshake(UI_API_VERSION, h.api_version); setHandshake(hs.level === "ok" ? null : hs.message); }).catch(() => undefined); };
      ws.onmessage = (ev) => {
    ...
    <ErrorBoundary>
      <div className="console">
        ...
        <StatusBar state={c?.state ?? "disconnected"} backend={c?.backend ?? "none"} pollHz={pollHz} reachable={reachable}
          ...
          recActive={status?.recording.active ?? false} recRun={status?.recording.run ?? null} printState={r?.state ?? "idle"} version={version} build={opBuild} platform={opPlatform} />
