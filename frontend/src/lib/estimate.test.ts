...
test("estimate matches the backend golden for the default plan (#6 FE/backend determinism)", () => {
  // Pinned to backend estimate_duration_s(PrintSettings(), 0.25) == 391.4 (build_backlash_mm default
  // is 0 — the attached piston measured zero backlash post settle-race fix). The two implementations
