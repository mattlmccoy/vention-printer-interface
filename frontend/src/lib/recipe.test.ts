...
import { DEFAULT_PLAN, compileRecipe, describeStep, totalLayers, totalThickness, validate } from "./recipe.ts";
...
test("compile of the full default plan: 12 layers, heights, resets", () => {
  const steps = compileRecipe({ ...DEFAULT_PLAN, n_heater_passes: 3 });
...
test("describeStep", () => {
  const s = compileRecipe(DEFAULT_PLAN);
