...
test("plain-language priming sequence: up, down, load, thick precoats", () => {
  const s = { part_top_mm: 0, feed_cavity_mm: 30, level_recoat_end_mm: 350, level_recoat_start_mm: 930, n_thick_precoats: 2, thick_feed_mm: 7 };
  ...
  assert.ok(/930/.test(thick[0]) && /350/.test(thick[0]) && /feed up 7/i.test(thick[0]));
});
