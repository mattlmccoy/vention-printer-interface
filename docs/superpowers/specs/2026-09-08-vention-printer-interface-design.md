...
**Renamed 2026-09-09:** the compiled layer loop and its settings are "print settings" and "the print" everywhere the operator sees them; the code renamed `recipe*`→`print_settings`/`print_controller`/`PrintSettings`/`PrintController`/`compile_print`, and API `/api/recipe*`→`/api/print-settings` (GET/PUT) and `/api/print/{start,pause,resume,step,abort}`. Modules are draggable to reorder AND draggable to resize (per-view pixel overrides in `vpi.console.v1`; a reset control restores the class default). Cross-sections render as plain greyscale (white paper, black ink). The machine module overlays live markers on the lab wireframe at `frontend/public/machine/printer.png` when present (calibration in `public/machine/calibration.json`, click-to-calibrate in the UI), else the schematic elevation.
