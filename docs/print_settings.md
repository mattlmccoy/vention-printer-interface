...
The print settings is the lab's print script (`vention/python/V1.py`) turned into data plus a step machine.

## Plan (`control/print_settings.py`)
`PrintSettings` (frozen) = three `PhasePlan`s (`precoat`, `printing`, `postcoat`: layer thickness,
...
`PrintSettings.bounded(data, limits)` clamps every field into the safety limits (tighten-only);
...
## Compiler
`compile_print(plan)` → tuple of `Step(index, phase, layer, kind, axis, value, label,
...
## Step machine (`control/print_controller.py`)
...
FAULT or disarm aborts the print settings. `dry_run` skips heater steps; `single_step` pauses after every
blocking step until `step()`. Heater steps go through the ARM-gated `Controller.heater_on/off`.

## API
`GET/PUT /api/print settings` (plan + `bounds` + `validation` + `n_steps`), `POST /api/print settings/start
{dry_run, single_step, name, notes}` (409 if not armed or invalid), `pause`, `resume`, `step`,
`abort`. `status.print` = the step-machine snapshot. With `auto-log` on (default) a run is
opened on start and closed on done/abort/fault; `layers.csv` gets one row per completed layer.

## Commissioning
1. `POST /api/print settings/start {"dry_run": true, "single_step": true}` with `printing.n_layers = 2`
